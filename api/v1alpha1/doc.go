...
// only the export flavor (dataFoundation); which of its optional blocks is
// populated follows the referenced StorageCluster's spec.management — an
// external cluster carries externalDetails (operator-supplied payload) and
// leaves dataFoundation empty, a managed cluster carries dataFoundation and
// may omit externalDetails (the consuming add-on's hook produces the
// payload). A presence union carries no
