...
| Add-on subsystem | `internal/addons/{plan,render,oc,records,inputs,steps,nativecatalog}` | Post-install add-on apply: binding/profile expansion and input effects (`plan`, `inputs`), plan and manifest rendering (`render`), the `oc` apply boundary (`oc`), step helpers (`steps`), non-secret apply records (`records`), and the built-in catalog plus machine-local store (`nativecatalog`). |
