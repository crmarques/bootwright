...
| `ceph.cephadm.clusterSSH.user` | No | `cephadm` when any node `Machine` sets `access.rootLogin: revoke`; otherwise `root` | The cluster's **post-install** account: the OS user cephadm manages every host as (`--ssh-user`) and the account Bootwright connects as once a node is provisioned. A non-root value is provisioned by Bootwright on every topology node; see [Revoking root SSH on Ceph nodes](#revoking-root-ssh-on-ceph-nodes). |
