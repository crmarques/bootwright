...
| Converge `generated:` and context-storage entries, report gaps | `bootwright secret sync` |
...
`bootwright secret sync` creates missing entries declared as `generated:` and,
when `secretStorage.mode: context`, copies external `file:` entries into the
context secrets directory; it then reports declared secrets that still need
`bootwright secret set` and exits non-zero while any remain missing. External
`file:` entries, such as provider-host SSH keys under `~/.ssh`, must already
exist at their declared paths before they can be copied.
