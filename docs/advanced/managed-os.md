...
bootwright media list --checksums
...
The default `media list` reads file metadata only. Add `--checksums` when you
need each current SHA-256; that option reads every listed ISO in full and can
therefore take time proportional to the total stored media size.
