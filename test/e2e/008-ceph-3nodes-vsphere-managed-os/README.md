# 008 Ceph 3 Nodes vSphere Managed OS

This fixture provisions a Ceph-only 3-node lab on vCenter-managed vSphere
VMs. It uses Bootwright-managed RHEL installation through datastore-staged
virtual media before running the community (OSS) Ceph storage flow. Each
YAML file contains one desired-state object with short names so the fixture
stays easy to inspect.

The current fixture keeps only state supported by the current code path:

- one provided controller host named `bastion`
- three lean managed Ceph VMs named `ceph-0`, `ceph-1`, and `ceph-2`,
  created blank through the vCenter API on failure domain `dc1-zone-a`
- a tiny `ceph-node` vSphere profile: 1 vCPU, 2048 MiB RAM, a 16 GiB root
  disk, and two 1 GiB OSD data disks per VM (root device `/dev/sda` on the
  paravirtual SCSI controller)
- a managed `lab-dns` dnsmasq name-resolution service on the bastion, bound to
  `192.168.142.1`, which authoritatively serves lab records and forwards every
...
- `infra/providers/vsphere.yaml`: vCenter connection, failure domain, and
  lean VM profile.
...
- `infra/images/rhel-9-dvd.yaml` and `infra/profiles/rhel-9-ceph.yaml`: managed
  OS install inputs.
- `clusters/storage/ceph-vsphere/*.yaml`: Ceph cluster topology, placement
...
- Working upstream internet on the bastion and Ceph nodes. The Ceph nodes reach the
  community Ceph repository (`download.ceph.com`), the EPEL bootstrap RPM
  (`dl.fedoraproject.org`), and the cephadm container image
  ...
  vSphere portgroup carries egress. The OSS distribution adds no
...
`bootwright apply --stage infra --clusters ceph-vsphere --yes`, the required
pyvmomi-backed Ansible requirements, `mkksiso`, and firewall rules should be
installed or configured by Bootwright. Missing
host preparation is a Bootwright bug, not a manual prerequisite for this lab.

Create the SSH key used for the bastion:
...
dnsmasq on the bastion (`192.168.142.1`). Point the laptop resolver at the same
...
Use the DNS IP from `environment.yaml` if the lab network changes.
