...
**Symptom:** Re-running `bootwright apply --stage clusters` after generated `install-config.yaml` or `agent-config.yaml` changed fails with `cannot generate ISO image due to configuration errors`, or the agent ISO does not reflect the latest rendered inputs.
