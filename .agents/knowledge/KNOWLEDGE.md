...
| Hook execution internals: content+inputs+target digest for onChange, ad-hoc SSH inventory with scoped secrets and the Ceph post-install identity for storage targets, fixed output persistence paths, firstReachable run-until-success, exportDetails resolution, and plan-time cross-cluster DAG edges replacing the compiled DF attachment. | [addon-hook-runtime-internals.md](addon-hook-runtime-internals.md) |
