...
| Ceph storage / destroy | `Task failed: Incorrect sudo password` right after a successful "Storage clusters" destroy step; a later destroy retry then fails `Failed to connect to the host via ssh: cephadm@<host>: Permission denied (publickey,...)`; `Failed to lookup user cephadm`; `getpwnam(): name not found`; `Deauthorize the machine access key`; apply --converge-drifted afterwards performs no OS reinstall; a successful full destroy with `--skip-unreachable` is followed by `Refuse to reinstall a reachable managed OS Bootwright cannot authenticate to` on every root-revoked Ceph node | [destroy-scoping-and-sweeps.md](destroy-scoping-and-sweeps.md) |
...
| Ceph storage | `Assert declared Ceph services reached their daemon count`; `mgmt-gateway (N/M)` stuck partway with every other service full; `ceph orch ps` shows mgmt-gateway daemons only on non-mgr hosts; `CEPHADM_DAEMON_PLACE_FAIL`; `Cannot bind to IP 0.0.0.0 port 8443: [Errno 98] Address already in use`; `TCP Port(s) '0.0.0.0:8443' required for mgmt-gateway already in use`; `MGR_MODULE_ERROR`; `Module 'dashboard' has failed: Port 8080 not free` | [ceph-monitoring-service-specs.md](ceph-monitoring-service-specs.md) |
...
| Per-substrate live ownership verification on apply and destroy (libvirt XML marker, vSphere annotation + moid addressing, KubeVirt labels), rename-safe identity recording, shared DVD-cache exclusion from per-machine records, per-machine managed-OS destroy dispatch, why a cluster-scoped gate (the libvirt network) silently stops firing inside a per-machine role, and podman-label ownership for bastion containers. | [substrate-ownership-markers.md](substrate-ownership-markers.md) |
...
| cephadm monitoring-spec constraints: retention keys are Prometheus-only, no dashboard Loki wiring command exists, role-less services render only when authored, the mgmt-gateway/keepalive_only ingress spec shape, why mgmt-gateway daemons never start on mgr hosts unless the classic dashboard's SSL port is disabled first, and why RGW/NFS ingress TLS takes one combined `ssl_cert` field unlike mgmt-gateway's two separate fields. | [ceph-monitoring-service-specs.md](ceph-monitoring-service-specs.md) |
...
| Hook execution internals: content+inputs+target digest for onChange, ad-hoc SSH inventory with scoped secrets and the Ceph post-install identity for storage targets, one task-scoped materialized kubeconfig shared by playbook and manifest hooks, fixed output persistence paths, firstReachable run-until-success, exportDetails resolution, and plan-time cross-cluster DAG edges replacing the compiled DF attachment. | [addon-hook-runtime-internals.md](addon-hook-runtime-internals.md) |
...
| Task-playbook preambles cannot be factored (play keywords), the five deliberate preamble variants (bastion env lookup, reachability no-become, managed-OS any_errors_fatal, storage linear, machine-destroy three-play barriers), and why component-selection and kubevirt verb blocks stay per-playbook/per-role. | [ansible-playbook-preamble-variants.md](ansible-playbook-preamble-variants.md) |
...
| Filter unit tests live under the collection test tree because Ansible imports every Python module under plugins/filter as a runtime plugin; keeping test_*.py beside filters produces repeated missing-FilterModule loader warnings during ansible-lint. | [ansible-filter-test-placement.md](ansible-filter-test-placement.md) |
