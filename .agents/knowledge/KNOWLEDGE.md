...
| Ansible runtime quirks | set_fact value with {{ hostvars }}/ansible_* resolves empty or wrong; broke on ansible-core 2.21 bump; template captures not-yet-gathered node facts; works when evaluated later | [ansible-core-eager-setfact.md](ansible-core-eager-setfact.md) |
| Ansible runtime quirks | play frozen mid-task with no error or timeout; host mid-reboot or powered off; SSH banner printed then stall; `ConnectTimeout=15`; `ServerAliveInterval=15`; `ServerAliveCountMax=3`; `ssh_common_args` | [ansible-ssh-liveness-timeouts.md](ansible-ssh-liveness-timeouts.md) |
...
| Apply, destroy & drift engine | REGISTRY_AUTH_HTPASSWD_PATH panic; crash-loop under restart_policy; net.ipv4.ip_nonlocal_bind; squid refuses to run as root UID 1000; Address not found; Cannot assign requested address; Cannot find device; dns enable='no'; bind-dynamic grabs :53; dhcp-option=option:ntp-server IPv4 only | [infra-component-service-gotchas.md](infra-component-service-gotchas.md) |
| Apply, destroy & drift engine | too many colons; Port could not be cast to integer value; bracketRedfishHost; net.JoinHostPort; TestManagedProxyURLBracketsIPv6; urlencode leaves / unescaped; replace('/', '%2F'); userinfo percent-encoding | [url-authority-gotchas.md](url-authority-gotchas.md) |
...
| Distribution-table-driven Ceph packaging and its traps: pinning cephadm's upgrade-discovery image base, mgr registry-store credential rotation, the FIPS OpenSSL conflict from host community ceph-common on RHEL and the cephadm-shell boundary, community EPEL bootstrap, vendor license markers, and RHSM idempotence. | [ceph-distribution-packaging.md](ceph-distribution-packaging.md) |
...
| The OSD device ownership marker and the install/destroy/removal device gates, the reclaim path after a managed-OS reinstall, the root-disk validators, and the post-apply OSD readiness poll that prevents zero-OSD green applies. | [ceph-osd-device-safety.md](ceph-osd-device-safety.md) |
| What counts as a Ceph sub-object's immutable structural identity, the fail-safe live probes that gate the data-destroying --override rebuild, EC pool creation traps, and the structural-hash invariants for reconcilable edits. | [ceph-override-structural-rebuild.md](ceph-override-structural-rebuild.md) |
...
| The probe_existing/wait/marker predicate ladder: port-open detection, fail-closed unverifiable hosts, ownership vs hash-match separation, the force_rebuild fact that makes --override actually reinstall, stamp gating against foreign-host laundering, and bare-metal media eject via cleanupMediaRole. | [managed-os-install-gates.md](managed-os-install-gates.md) |
| The anaconda install role's fail-closed edges: reachable-but-unverifiable refusal, owned-vs-matching marker predicates, the force_rebuild/install_required fact chain, install_required-gated post-boot waits, ownership-gated marker stamping, and infra-stage remedy routing. | [managed-os-install-safety-gates.md](managed-os-install-safety-gates.md) |
...
| Single-source rules for shared flag help and the --stage/--through vocabulary, the --override help contract, cobra dispatcher wiring gotchas, the -- flag-terminator rule for JSON error output, and the unbuffered confirm() stdin gotcha. | [cli-flag-and-parsing-conventions.md](cli-flag-and-parsing-conventions.md) |
...
| Render-reference vs work-set semantics for --clusters scopes: nil-vs-empty StorageClusterNames, preflight secret scope, connected-machine host trust, state-check parity, full-state orphan detection, whole-input validation, service-machine retention. | [scoped-runs-render-vs-work-set.md](scoped-runs-render-vs-work-set.md) |
...
| Topic | File |
| --- | --- |
| Maps the state-engine audit-finding IDs (M2 lease takeover, M8 scope-dependent hash, M11 state-check --clusters parity) to the invariants they encode and the regression tests that pin them. | [audit-finding-regression-tests.md](audit-finding-regression-tests.md) |
