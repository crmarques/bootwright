...
| Ceph storage | `Assert declared Ceph services reached their daemon count`; `mgmt-gateway (N/M)` stuck partway with every other service full; `ceph orch ps` shows mgmt-gateway daemons only on non-mgr hosts; `CEPHADM_DAEMON_PLACE_FAIL`; `Cannot bind to IP 0.0.0.0 port 8443: [Errno 98] Address already in use`; `TCP Port(s) '0.0.0.0:8443' required for mgmt-gateway already in use` | [ceph-monitoring-service-specs.md](ceph-monitoring-service-specs.md) |
