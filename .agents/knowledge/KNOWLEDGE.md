...
| Ceph storage | `Assert declared Ceph services reached their daemon count`; `mgmt-gateway (N/M)` stuck partway with every other service full; `ceph orch ps` shows mgmt-gateway daemons only on non-mgr hosts | [ceph-monitoring-service-specs.md](ceph-monitoring-service-specs.md) |
