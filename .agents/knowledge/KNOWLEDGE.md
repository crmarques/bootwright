...
| How plaintext secret copies on disk are kept short-lived: post-lease sweep reclaims SIGKILL orphans, store materialization is all-or-nothing, cluster stores target migration around the nested add-ons area, MkdirAll needs explicit Chmod, and preflight rejects only over-broad modes. | [plaintext-secret-lifecycle.md](plaintext-secret-lifecycle.md) |
