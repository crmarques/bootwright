...
## B-050 — Only `deviceName` of the eight rootDeviceHints fields reaches the Anaconda kickstart
- Status: open
- Area: managed-os / partitioning
- Origin: ceph-prd-01 zero-OSD investigation 2026-07-29 (adjacent finding, not causal)
- Severity: medium
- Problem: `machineInstallStorageVars`
  (`internal/render/inventory/vars_machine_os_install.go:288`) reads only
  `hints.DeviceName`. A Machine that identifies its root disk by `wwn`,
  `serialNumber`, `model`, `vendor`, `hctl`, `minSizeGigabytes` or `rotational`
  — all first-class fields, and all honoured on the OCP agent-installer path
  via `RootDeviceHintsConfig` — renders `storage.rootDisk` empty and drops into
  the `ks.cfg.j2:90-93` else branch: `clearpart --all --initlabel` plus
  `autopart --type=lvm`, which LVM-spans *every* disk on the machine and emits
  no `bootloader` line. `docs/advanced/managed-os.md:378-380` promises all eight
  hints are honoured. Ceph OSD nodes are protected by
  `validateManagedOSCephNodeRootDisk`, which fires on exactly the blank-`deviceName`
  case, so this cannot produce a zero-OSD Ceph install; the exposure is every
  OTHER managed-OS machine — bastions, hub hosts, RHEL workers — where a
  stable-by-id root hint silently becomes an all-disk wipe.
- Exit: resolve the non-`deviceName` hints into an Anaconda-usable selector
  (`ignoredisk --only-use=` and `clearpart --drives=` both accept
  `disk/by-id/...`, so `wwn`/`serialNumber` can render directly), or reject them
  at validation for machines whose install profile uses the anaconda installer
  rather than silently taking the destructive fallback. Pin with a test.
- Related: [ceph-osd-device-safety.md](ceph-osd-device-safety.md),
  [managed-os-install-gates.md](managed-os-install-gates.md)
