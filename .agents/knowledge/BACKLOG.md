...
- Problem: the staticcheck stage fails before it can validate later changes.
  ...
  `internal/repo/checks/go_mutating_invocation_test.go` use double-quoted
  literals that require avoidable backslash escaping (S1007).
...
## B-091 — The add-on readiness deadline test races under `-race`
...
- Origin: `make check-full` for commit 74ad0eb3 on 2026-08-10; reproduced on
  merge-base ed51b82d
- Severity: low
- Problem: `TestApplyAndWaitShareOneReadinessDeadline` exercises concurrent
  readiness probes through `phasedCSVRunner`, but that test runner reads and
  appends its call-tracking slices without synchronization. The production
  concurrency is intentional; the fake itself makes the release race gate red.
- Exit: make the test runner's phase selection and call recording
  concurrency-safe without serializing the behavior under test, make the race
  test green, and delete this entry in the same fix.

## B-092 — The clean-checkout test requires an excluded bundle artifact

- Status: in-progress
- Area: repository fitness / tests
- Origin: `make check-full` for commit 74ad0eb3 on 2026-08-10; reproduced on
  merge-base ed51b82d
- Severity: low
- Problem: `go-test-clean-checkout` deliberately excludes the gitignored
  `internal/converge/bundle/ansible_bundle.zip`, but the authorized shared-infra
  case in `TestApplyDestroySafetyMatrix` reaches bundle preparation and fails
  with `embedded ansible bundle is empty ... missing ansible_bundle.zip` before
  it reaches the intended fake mutation.
- Exit: make the safety-matrix mutation hermetic without relying on the local
  generated bundle, preserve the clean-checkout exclusion, make the gate green,
  and delete this entry in the same fix.
