# Ceph --override structural rebuilds: identity, fail-safes, and EC pools
...
warrants a data-destroying `--override` rebuild, because Ceph cannot change it
...
**Semantics:** A rebuild runs only under `--override` AND on a proven
...
move it — otherwise apply refuses the edit or `--override` wipes the cluster's
