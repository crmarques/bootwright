...
| a `cephadm shell` argv | structured `ansible.builtin.command.argv`, its semantic class in `cephShellStaticClass` (or the closed dynamic-argv switch), the matching role-default timeout var, and the vars-contract class table; every local or enclosing failure/retry/loop path must propagate rc 124/137, and `no_log` commands require the shared non-secret timeout relay | `TestAnsibleBoundsEveryCephadmShellCommand` |
...
| a declaration-removal or prune path | an explicit target-bearing state-changing operation with preview, authorization classification, exact retry, and a safety-matrix row; never infer consent from a declaration absent during `apply` | `TestApplyOmissionStaysNonDestructive` |
