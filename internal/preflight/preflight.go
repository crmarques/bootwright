...
func CollectChecks(state v1alpha1.State, selected []Phase, hasState bool, contextName, secretsDir, clustersDir, runtimeDir string, deps Deps, hostTrustScope map[string]bool, secretScope *SecretScope) []Check {
	...
	if hasState {
		...
		checks = append(checks, generatedSelfSignedDriftChecks(state, contextName, secretsDir)...)
		checks = append(checks, kubeVirtHostClusterChecks(state, selected, contextName, clustersDir, secretsDir, runtimeDir, deps, secretScope)...)
