...
func TestInventoryWithOwnershipRecordsAddsRecordedHost(t *testing.T) {
	...
	inv := render.InventoryWithOwnershipRecords(v1alpha1.State{}, "", records)
	...
	infraHosts := children[render.GroupInfraHosts].(map[string]any)["hosts"].(map[string]any)
	if _, ok := infraHosts["provider-0"]; !ok {
		t.Fatalf("%s hosts = %v, want provider-0", render.GroupInfraHosts, infraHosts)
	}
	counts := render.HostGroupCountsWithOwnershipRecords(v1alpha1.State{}, records)
	if got := counts[render.GroupInfraHosts]; got != 1 {
		t.Fatalf("%s count = %d, want 1", render.GroupInfraHosts, got)
	}
}
