...
func TestInventoryUsesLocalhostForControllerWork(t *testing.T) {
	...
	groupHosts := children[GroupControllerHosts].(map[string]any)["hosts"].(map[string]any)
	if _, ok := groupHosts["localhost"]; !ok {
		t.Fatalf("%s hosts = %v, want localhost", GroupControllerHosts, groupHosts)
	}
}
...
func TestInventoryKeepsLibvirtProviderHostsForContextDestroy(t *testing.T) {
	...
	children := all["children"].(map[string]any)
	providerHosts := children[GroupProviderHosts].(map[string]any)["hosts"].(map[string]any)
	if _, ok := providerHosts["unused-host"]; !ok {
		t.Fatalf("%s must include libvirt provider host: %v", GroupProviderHosts, providerHosts)
	}
}
...
func TestHostGroupCountsBareMetalManagedArtifacts(t *testing.T) {
	...
	if got := counts[GroupProviderHosts]; got != 0 {
		t.Errorf("%s: want 0 for bare-metal provider work, got %d", GroupProviderHosts, got)
	}
	if got := counts[GroupInfraComponentHosts]; got != 1 {
		t.Errorf("%s: want 1 for bare-metal managed artifacts, got %d", GroupInfraComponentHosts, got)
	}
	if got := counts[GroupInfraHosts]; got != 0 {
		t.Errorf("%s: want 0 for bare-metal substrate, got %d", GroupInfraHosts, got)
	}
	if got := counts[GroupBootHosts]; got != 1 {
		t.Errorf("%s: want 1 for Redfish artifact staging, got %d", GroupBootHosts, got)
	}
	if got := counts[GroupOCPHosts]; got != 1 {
		t.Errorf("%s: want 1 for bastion host, got %d", GroupOCPHosts, got)
	}
	if got := counts[GroupAgentNodeHosts]; got != 1 {
		t.Errorf("%s: want 1 for one agent node host, got %d", GroupAgentNodeHosts, got)
	}
}
...
func TestBareMetalCorporateFixtureInventoriesOnlyBastionServices(t *testing.T) {
	...
	counts := HostGroupCounts(state)
	if got := counts[GroupProviderHosts]; got != 0 {
		t.Errorf("%s: want 0 for direct bare-metal provider work, got %d", GroupProviderHosts, got)
	}
	if got := counts[GroupInfraComponentHosts]; got != 1 {
		t.Errorf("%s: want 1 for bastion artifact publication only, got %d", GroupInfraComponentHosts, got)
	}
	if got := counts[GroupInfraHosts]; got != 0 {
		t.Errorf("%s: want 0 for direct bare-metal machines, got %d", GroupInfraHosts, got)
	}
	if got := counts[GroupBootHosts]; got != 1 {
		t.Errorf("%s: want 1 for Redfish artifact staging, got %d", GroupBootHosts, got)
	}
	if got := counts[GroupAgentNodeHosts]; got != 3 {
		t.Errorf("%s: want 3 for agent node fanout, got %d", GroupAgentNodeHosts, got)
	}
	...
	children := all["children"].(map[string]any)
	groupName := AgentNodeGroupName("3-nodes-ocp-baremetal")
...
func TestHostGroupCountsLibvirtManaged(t *testing.T) {
	...
	counts := HostGroupCounts(state)
	if got := counts[GroupProviderHosts]; got == 0 {
		t.Errorf("%s: want >0 for provider setup fixture, got 0", GroupProviderHosts)
	}
	if got := counts[GroupInfraComponentHosts]; got == 0 {
		t.Errorf("%s: want >0 for managed-services fixture, got 0", GroupInfraComponentHosts)
	}
	if got := counts[GroupInfraHosts]; got == 0 {
		t.Errorf("%s: want >0 for libvirt-substrate fixture, got 0", GroupInfraHosts)
	}
	if got := counts[GroupBootHosts]; got == 0 {
		t.Errorf("%s: want >0 for Redfish boot delegation, got 0", GroupBootHosts)
	}
}
...
func TestStorageInventoryUsesManagedCephHosts(t *testing.T) {
	...
	inv := Inventory(state, secretsDir)
	all := inv["all"].(map[string]any)
	hosts := all["hosts"].(map[string]any)
	seedName := StorageSeedHostName(state.StorageClusters[0])
	...
	nodeName := StorageNodeHostName("ceph-storage", "ceph-dc1-1")
	...
	if _, ok := groupHosts[seedName]; !ok {
		t.Fatalf("%s hosts = %v, want %s", GroupStorageHosts, groupHosts, seedName)
	}
	if _, ok := groupHosts[nodeName]; !ok {
		t.Fatalf("%s hosts = %v, want %s", GroupStorageHosts, groupHosts, nodeName)
	}
	clusterGroup := StorageClusterGroupName("ceph-storage")
	...
	counts := HostGroupCounts(state)
	if got := counts[GroupStorageHosts]; got != 7 {
		t.Fatalf("%s count = %d, want 7", GroupStorageHosts, got)
	}
