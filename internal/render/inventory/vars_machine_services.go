...
func loadBalancerMachineServiceVars(state v1alpha1.State, service stategraph.MachineService) (map[string]any, bool) {
	...
	for _, consumer := range service.Consumers {
		...
		frontends = append(frontends, loadBalancerFrontends(state, ci, component.Metadata.Name, consumer.Cluster, ci.Machines, stateview.ClusterNodesForInstall(state, ci))...)
	}
	if len(frontends) > 0 {
...
func providerMachineSetupsVars(state v1alpha1.State) []any {
	...
	for _, k := range keys {
		out = append(out, map[string]any{
			...
			"machineAddress": stateview.MachineSSHAddressByName(state, k.machine),
			"applyRole":      k.role,
		})
	}
	return out
