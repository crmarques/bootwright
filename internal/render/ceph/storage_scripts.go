...
func CephApplyScript(state v1alpha1.State, cluster v1alpha1.StorageCluster, opts CephScriptOptions) (string, error) {
	...
	b.WriteString("# controller-side convergence. Re-runnable: create-style steps are guarded and\n")
	b.WriteString("# skip-if-exists; set-* steps reconcile in place. The data-destroying\n")
	b.WriteString("# The --mode rebuild path is bootwright-only and is not generated here.\n")
	...
	if MgmtGatewayHasSecrets(cluster) {
		...
		b.WriteString("echo \"  [todo] mgmt-gateway spec with your cert/secret files, or reconcile it through Bootwright from the controller context.\"\n")
	}
