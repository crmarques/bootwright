...
func dataFoundationCredentialOperations(state v1alpha1.State, cluster v1alpha1.StorageCluster) []map[string]any {
	...
	for _, attachment := range storageAttachmentsByStorageCluster(state)[cluster.Metadata.Name] {
		...
		ops = append(ops,
			...
			dataFoundationCephAuthOperation(containerCluster, "cephFSNodeKey", "create-data-foundation-cephfs-node-"+containerCluster, "ceph", "auth", "get-or-create", "client."+cephFSNode, "mon", "allow r", "mgr", "allow rw", "mds", "allow rw", "osd", "allow rw tag cephfs data="+df.FilesystemRef.Name),
			dataFoundationCephAuthOperation(containerCluster, "cephFSProvisionerKey", "create-data-foundation-cephfs-provisioner-"+containerCluster, "ceph", "auth", "get-or-create", "client."+cephFSProvisioner, "mon", "allow r", "mgr", "allow rw", "mds", "allow rw", "osd", "allow rw tag cephfs data="+df.FilesystemRef.Name),
		)
