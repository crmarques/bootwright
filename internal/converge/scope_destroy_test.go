...
func TestApplyDestroyScopeExtraVarsStorageGate(t *testing.T) {
	...
	t.Run("no narrowing emits nothing", func(t *testing.T) {
		...
		ApplyDestroyScopeExtraVars(&plan, false, "", nil, nil, false, false, false, false)
	...
	t.Run("container-only emits empty allowlist (tear down none)", func(t *testing.T) {
		...
		ApplyDestroyScopeExtraVars(&plan, false, "ocp-a", nil, nil, false, false, false, false)
	...
	t.Run("storage-narrowed emits the named roots", func(t *testing.T) {
		...
		ApplyDestroyScopeExtraVars(&plan, false, "ceph-a,ceph-b", nil, nil, false, false, false, false)
		val, ok := storageVar(plan)
...
func TestApplyDestroyScopeExtraVarsSkipUnreachable(t *testing.T) {
	...
	t.Run("emitted on opt-in", func(t *testing.T) {
		plan := WorkflowPlan{}
		ApplyDestroyScopeExtraVars(&plan, false, "", nil, nil, false, false, true, false)
		if !has(plan) {
...
func TestApplyDestroyScopeExtraVarsUnownedSplit(t *testing.T) {
	...
	t.Run("vms only", func(t *testing.T) {
		plan := WorkflowPlan{}
		ApplyDestroyScopeExtraVars(&plan, true, "", nil, nil, true, false, false, false)
	...
	t.Run("networks only", func(t *testing.T) {
		plan := WorkflowPlan{}
		ApplyDestroyScopeExtraVars(&plan, true, "", nil, nil, false, true, false, false)
		if !has(plan, DestroyAuthorizeUnownedNetsExtraVar) {
	...
	t.Run("devices are a third axis, authorized by neither VM token", func(t *testing.T) {
		plan := WorkflowPlan{}
		ApplyDestroyScopeExtraVars(&plan, true, "", nil, nil, true, true, false, false)
		if has(plan, CephAuthorizeUnownedDevicesExtraVar) {
			t.Fatalf("authorizing unowned VMs and networks must not authorize wiping a device this node has no OSD record for (ADR 0034); got %v", plan.ExtraVarPairs)
		}
		devices := WorkflowPlan{}
		ApplyDestroyScopeExtraVars(&devices, true, "", nil, nil, false, false, false, true)
		if !has(devices, CephAuthorizeUnownedDevicesExtraVar) {
			t.Fatalf("--authorize unowned-devices must emit %s=true; got %v", CephAuthorizeUnownedDevicesExtraVar, devices.ExtraVarPairs)
		}
	})
}
