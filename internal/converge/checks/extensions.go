...
func ExtensionPreflight(clustersDir string, state v1alpha1.State, deps ExtensionDeps) []Check {
	...
	for _, plan := range plans {
		...
		switch {
		case err != nil:
			checks = append(checks, failCheck("Cluster access", plan.Cluster+" kubeconfig", path+" missing", "Addons need the installed cluster kubeconfig", "run bootwright apply cluster --yes before applying addons"))
		case info.IsDir():
