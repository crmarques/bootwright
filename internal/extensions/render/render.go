...
func OLMResources(extension v1alpha1.ClusterExtension) ([]ManifestResource, error) {
...
func DesiredHash(extension v1alpha1.ClusterExtension, policy v1alpha1.ClusterExtensionPolicy) (string, error) {
	...
	payload := struct {
		...
		Extension  v1alpha1.ClusterExtension `json:"extension"`
	...
	case v1alpha1.ClusterExtensionTypeManifestSet:
		for _, manifest := range extension.Spec.ManifestSet.Manifests {
			...
			if err != nil {
				return "", fmt.Errorf("read ClusterExtension/%s manifest %s: %w", extension.Metadata.Name, manifest.Path, err)
			}
	...
	if err != nil {
		return "", fmt.Errorf("encode ClusterExtension/%s desired hash input: %w", extension.Metadata.Name, err)
	}
...
func ManifestPath(extension v1alpha1.ClusterExtension, manifest v1alpha1.ClusterExtensionManifestRef) string {
