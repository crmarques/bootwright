...
func TestDesiredHashTracksManifestContent(t *testing.T) {
	...
	first, err := DesiredHash(extension, policy)
	if err != nil {
		t.Fatalf("DesiredHash first: %v", err)
	}
	second, err := DesiredHash(extension, policy)
	if err != nil {
	...
	changed, err := DesiredHash(extension, policy)
	if err != nil {
...
func TestDesiredHashTracksOLMCustomResourceChanges(t *testing.T) {
	...
	firstHash, err := DesiredHash(first, policy)
	if err != nil {
		t.Fatalf("DesiredHash first: %v", err)
	}
	secondHash, err := DesiredHash(second, policy)
	if err != nil {
		t.Fatalf("DesiredHash second: %v", err)
	}
...
func TestReadinessChecks(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			ready, observed, err := Ready(context.Background(), runner, "/tmp/kubeconfig", tc.extension)
			if err != nil {
...
func TestReadinessTimeoutReportsLastObservedState(t *testing.T) {
	...
	last, err := WaitReady(context.Background(), runner, "/tmp/kubeconfig", extension, time.Millisecond)
	if err == nil {
