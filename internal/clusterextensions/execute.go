...
func Apply(ctx context.Context, runner OCRunner, cfg RunConfig, plan ExtensionPlan) (TaskResult, error) {
	if err := requireKubeconfig(cfg.Kubeconfig); err != nil {
		return TaskResult{}, err
	}
	hash, err := DesiredHash(plan.Extension, plan.Policy)
	if err != nil {
		return TaskResult{}, err
	}
	if ready, _, err := Ready(ctx, runner, cfg.Kubeconfig, plan.Extension); err == nil && ready {
		record, found, err := LoadRecord(cfg.RuntimeDir, plan.Cluster, plan.Name)
		if err != nil {
			return TaskResult{}, err
		}
	...
	record := Record{
		...
		Extension:   plan.Name,
		DesiredHash: hash,
		Status:      RecordStatusApplying,
		Phase:       RecordPhaseApplying,
		RunID:       cfg.RunID,
		StartedAt:   cfg.StartedAt.UTC(),
		UpdatedAt:   now,
	}
	if err := SaveRecord(cfg.RuntimeDir, record); err != nil {
		return TaskResult{}, err
	}
	...
	if err != nil {
		...
		_ = SaveRecord(cfg.RuntimeDir, record)
		return TaskResult{}, err
	}
	record.Status = RecordStatusWaiting
	record.Phase = RecordPhaseApplied
...
func Wait(ctx context.Context, runner OCRunner, cfg RunConfig, plan ExtensionPlan) (TaskResult, error) {
	if err := requireKubeconfig(cfg.Kubeconfig); err != nil {
		return TaskResult{}, err
	}
	hash, err := DesiredHash(plan.Extension, plan.Policy)
	...
	if found && record.DesiredHash == hash && record.Status == RecordStatusReady {
	...
	record.Status = RecordStatusWaiting
	record.Phase = RecordPhaseWaiting
	...
	record.Status = RecordStatusReady
	record.Phase = RecordPhaseComplete
	if err := SaveRecord(cfg.RuntimeDir, record); err != nil {
		return TaskResult{}, err
	}
	return TaskResult{}, nil
...
func applyExtension(ctx context.Context, runner OCRunner, kubeconfig string, plan ExtensionPlan) ([]string, error) {
	switch plan.Extension.Spec.Type {
	case v1alpha1.ClusterExtensionTypeOLMOperator:
		resources, err := OLMResources(plan.Extension)
		if err != nil {
			return nil, err
		}
		var observed []string
		for _, resource := range resources {
			...
			observed = append(observed, ObservedResourceID(resource.Kind, resource.Namespace, resource.Name))
		}
		return observed, nil
	case v1alpha1.ClusterExtensionTypeManifestSet:
		var observed []string
		for _, manifest := range plan.Extension.Spec.ManifestSet.Manifests {
			path := ManifestPath(plan.Extension, manifest)
