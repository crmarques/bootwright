...
func TestAnsibleCollectionLockMatchesEmbeddedManifest(t *testing.T) {
	archive, err := openBundleArchive(t)
	if err != nil {
		t.Skip("embedded collection bundle has not been synced")
	}
	files := archiveFiles(t, archive)
	if !archiveHasPrefix(files, "collections/ansible_collections/") {
		t.Skip("embedded collection bundle has not been synced")
	}
	...
	for _, requirement := range requirements.Collections {
		...
		manifestPath := filepath.ToSlash(filepath.Join("collections/ansible_collections", namespace, name, "MANIFEST.json"))
		manifestData, ok := files[manifestPath]
		if !ok {
			t.Fatalf("embedded bundle missing %s", manifestPath)
		}
...
func openBundleArchive(t *testing.T) (*zip.ReadCloser, error) {
	t.Helper()
	archive, err := zip.OpenReader(filepath.Join(repoRoot(t), "internal/converge/bundle/ansible_bundle.zip"))
	if err != nil {
		return nil, err
	}
	return archive, nil
...
func archiveFiles(t *testing.T, archive *zip.ReadCloser) map[string][]byte {
	t.Helper()
	defer archive.Close()
	out := map[string][]byte{}
	for _, f := range archive.File {
		...
		r, err := f.Open()
		if err != nil {
			t.Fatalf("open archived %s: %v", f.Name, err)
		}
...
func archiveHasPrefix(files map[string][]byte, prefix string) bool {
