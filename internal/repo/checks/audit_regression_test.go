...
func TestGetURLDownloadsPinChecksums(t *testing.T) {
	acceptedUnpinnable := map[string]string{
		...
		bootwrightCollectionRoleRoot + "/controller_openshift_tools/tasks/main.yml\x00Download helm checksums":                         "this IS the sha256sum.txt manifest the helm binary fetch pins against; pinning it would be circular",
