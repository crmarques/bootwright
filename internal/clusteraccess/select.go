...
func FormatStorageConsumerConflicts(conflicts []stategraph.StorageConsumerConflict, retryCommand string) error {
	...
	b.WriteString("include the consuming cluster(s) in --clusters, destroy them first")
	if retryCommand != "" {
		b.WriteString(" with `" + retryCommand + "`")
	}
	b.WriteString(", or remove the storage attachment binding if the consumer is already gone")
