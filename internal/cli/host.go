...
func newHostTrustCmd(stdin io.Reader, stdout io.Writer, _ io.Writer) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "trust",
		Short: "Record SSH host-key trust for declared Hosts",
		Args:  cobra.NoArgs,
  ...
  bootwright host trust --output json --dry-run`,
	...
	cmd.Flags().StringVar(&hosts, "hosts", "", "comma-separated Host names to trust")
	cmd.Flags().StringVar(&replace, "replace", "", "comma-separated Host names whose changed trust may be replaced")
...
func runHostTrust(ctx context.Context, stdin io.Reader, stdout io.Writer, contextName, contextDir string, state v1alpha1.State, opts hostTrustOptions, deps hostTrustDeps) (hostTrustReport, error) {
	...
	if len(plan.records) > 0 || len(managedTrustHosts(state, controllerLocalityPolicy)) > 0 {
...
func hostTrustNeedsScan(state v1alpha1.State, selected map[string]bool) bool {
	for _, host := range state.Hosts {
		if len(selected) > 0 && !selected[host.Metadata.Name] {
			continue
		}
		if host.Spec.SSH == nil || host.Spec.SSH.KnownHostsRef.Name != "" || locality.IsControllerLocalHost(host, controllerLocalityPolicy) {
			continue
		}
		return true
	}
...
func buildHostTrustPlan(ctx context.Context, state v1alpha1.State, store sshtrust.Store, selected, replace map[string]bool, scan func(context.Context, string, time.Duration) ([]sshtrust.ScannedKey, error)) (hostTrustPlan, error) {
	...
	hosts := append([]v1alpha1.Host(nil), state.Hosts...)
	sort.Slice(hosts, func(i, j int) bool { return hosts[i].Metadata.Name < hosts[j].Metadata.Name })
	seen := map[string]bool{}
	for _, host := range hosts {
		if len(selected) > 0 && !selected[host.Metadata.Name] {
			continue
		}
		seen[host.Metadata.Name] = true
		report, record, write, err := evaluateHostTrust(ctx, host, store, replace[host.Metadata.Name], scan)
		if err != nil {
			return plan, err
		}
...
func evaluateHostTrust(ctx context.Context, host v1alpha1.Host, store sshtrust.Store, replace bool, scan func(context.Context, string, time.Duration) ([]sshtrust.ScannedKey, error)) (hostTrustHostReport, sshtrust.HostRecord, bool, error) {
	report := hostTrustHostReport{Name: host.Metadata.Name}
	if host.Spec.SSH == nil {
		report.Action = "skip"
		report.Reason = "Host has no spec.ssh"
		return report, sshtrust.HostRecord{}, false, nil
	}
	report.Address = v1alpha1.HostSSHAddress(host)
	if locality.IsControllerLocalHost(host, controllerLocalityPolicy) {
		report.Action = "skip"
		report.Reason = "controller-local Host"
		return report, sshtrust.HostRecord{}, false, nil
	}
	...
	if err != nil {
		return report, sshtrust.HostRecord{}, false, fmt.Errorf("scan Host/%s at %s: %w", host.Metadata.Name, report.Address, err)
	}
	key, ok := sshtrust.SelectPreferred(keys)
	if !ok {
		return report, sshtrust.HostRecord{}, false, fmt.Errorf("scan Host/%s at %s: no SSH host keys returned", host.Metadata.Name, report.Address)
	}
	...
	previous, exists := store.Find(host.Metadata.Name)
	if !exists {
		report.Action = "add"
