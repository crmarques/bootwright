...
func printDestroyRecordReset(stdout io.Writer, sel clusteraccess.Selection, runsDir, clustersDir string, runScope converge.Scope, plan converge.WorkflowPlan, resetPartial []string, succeeded map[string]bool) {
	if sel.MachineSelection {
		printConvergeRecordResetProblems(stdout, converge.ResetMachineConvergeRecordsAfterDestroy(runsDir, plan.State, sel.MachineProvision, succeeded))
		return
	}
	printConvergeRecordResetProblems(stdout, converge.ResetConvergeRecordsAfterDestroy(runsDir, clustersDir, runScope, plan.State, plan.StorageWorkNames, resetPartial, succeeded))
}
