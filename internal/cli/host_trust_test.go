...
func TestHostTrustPreflightFailsWhenManagedTrustMissing(t *testing.T) {
	...
	checks := hostTrustChecks(state, "/context/secrets", []Phase{{Name: "cluster-infra"}}, preflightDeps{
	...
	if !hasCheck(checks, checkGroupHostTrust, "Host/provider-01", "bootwright host trust") {
...
`,
		"host.yaml": `apiVersion: bootwright.io/v1alpha1
kind: Host
metadata:
  name: provider-01
spec:
...
func hostTrustTestState() v1alpha1.State {
	return v1alpha1.State{Hosts: []v1alpha1.Host{{
		Metadata: v1alpha1.Metadata{Name: "provider-01"},
		Spec: v1alpha1.HostSpec{
			Addresses: []v1alpha1.HostAddress{{Name: "ssh", Address: "provider-01.example.test"}},
