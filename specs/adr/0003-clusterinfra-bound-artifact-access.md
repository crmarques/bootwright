# ClusterInfra-Bound Artifact Access
...
hosts; `ClusterInfra` chooses which service endpoint each install consumer
uses for a given cluster infrastructure.
...
Catalog artifact servers in `Environment.spec.infraComponents.artifactServers[]`.
Managed entries reference an `InfraComponent`; external entries declare named
URLs. Bind consumer audiences in `ClusterInfra.spec.artifactAccess`:

- `serverRef.name` selects the environment artifact server catalog entry.
- `redfishVirtualMedia.endpointRef.name` selects the endpoint rendered into
  BMC ISO fetch URLs.
- `containerClusterInstall.endpointRef.name` selects the endpoint rendered
  into disconnected `bootArtifactsBaseURL`.
...
- `InfraComponent` owns artifact server placement, listener, and endpoint
  definition.
- `Environment` owns artifact server catalog entries.
- `ClusterInfra` owns route binding for install consumer audiences while not
  owning generated file serving.
