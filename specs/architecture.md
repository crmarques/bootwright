...
Absent explicit cluster-install or Redfish throttles, the selected dependency
DAG, resource locks, and the global and per-host safety budgets are the only
ordering constraints. Default Redfish capacity equals the selected apply or
destroy graph's declared demand and adds no ordering. Independent bare-metal
ContainerCluster roots may therefore enter their install chains together while
peer Ceph machine and storage work runs; neither cluster kind imposes an
undeclared edge on the other.
